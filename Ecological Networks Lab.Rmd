...
Reflection 1.x. Connecting network definition to. If it loses all in-degree, it would go extinct by secondary extinction. Not losing all links per say, specificaly about in-degree.

Reflection 1.x. Species 1 has lots of in-degree, species 2 has very few in-degree. Which one is more vulnerable to extinction?
...
Reflection 1.3. Small world networks. Random (every species has approx. the same number of links) vs highly skewed distributions. Easier to imagine. Insert figure visualizing both random and highly skewed distributions. Degree distribution is a hard concept overall, make sure it's very clear. Frame it as "the probability of interacting" with other species, which will help with thinking about extinction/removal of those links.

(hard reflection) How do you hypothesize that networks with power law vs exponential degree distributions might respond differently to the removal/extinction of most-connected nodes?
...
#Show a subset of the columns. Something like just S, L, C, secExt columns. If you want to explore other columns, you can do it in your own time. We might not have time to understand prection release, modularity, etc. 

#students will be moving at different paces. providing resources for people to move forward at their own pace while keeping everything as simple as possible in the main materials. 
```

Reflection 2.x. Looking at the figure, we're moving most to least connected species. Even with that, the network is very robust. Good reflection b/c you can integrate ecological concepts. In the intertidal, it's because there's a lot of omnivory with highly generalist and redundant interactions. This is one reason why this network is so robust. In the Scientific Reports paper, there is a section where they discuss why the network is so robust.
...
Important to highlight: the number of simulations could be very low. running code in parallel, depends on each person's computer. Can reduce the number of simulations from 100 to run without parallel.
